...
{/if}
{assign var=hasReviews value=(bool) $row->getReviews()}

{assign var="row_class" value="gridRow"}
{if $hasReviews || $row->getActions(\PKP\controllers\grid\GridHandler::GRID_ACTION_POSITION_DEFAULT)}
...
<tr {if $rowId}id="{$row_id}" {/if} class="{$row_class}">
	{foreach name=columnLoop from=$columns key=columnId item=column}
		...
		<td{if $col_class} class="{$col_class}" {/if}>
			{if ($hasReviews || $row->hasActions()) && $column->hasFlag('firstColumn')}
				{if $hasReviews || $row->getActions(\PKP\controllers\grid\GridHandler::GRID_ACTION_POSITION_DEFAULT)}
